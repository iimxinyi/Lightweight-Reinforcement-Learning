...
class ReplayBuffer:
    def __init__(self, para, max_step, observation_dim):
    ...
    def reset_buffer(self):
        self.position = 0
        self.buffer = {'observation': np.zeros([self.batch_size, self.max_step, self.observation_dim]),
    ...
    def store(self, step, observation, observation_value, action, action_logprob, reward, done, truncated):
        index = self.position % self.batch_size
        self.buffer['observation'][index][step] = observation
...
class IPPO:
    def __init__(self, para, max_step, observation_dim, action_dim):
        self.max_step = max_step
        self.observation_dim = observation_dim
        ...
        self.critic_input_dim = self.observation_dim
        ...
        self.replay_buffer = ReplayBuffer(para, self.max_step, self.observation_dim)
