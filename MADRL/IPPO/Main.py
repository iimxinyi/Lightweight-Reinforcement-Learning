...
def train(para):
    ...
    for agent in range(para.num_agent):
        agent_network.append(IPPO(para, para.STEP, observation_dim, action_dim))
    ...
    for episode in range(para.EPISODE):
        ...
        for step in range(para.STEP):
            ...
            for agent in range(para.num_agent):
                action, action_logprob = agent_network[agent].choose_action(all_observation[agent])
                all_action.append(action)
                all_action_logprob.append(action_logprob)
                observation_value = agent_network[agent].get_observation_value(all_observation[agent])
            ...
            for agent in range(para.num_agent):
                agent_network[agent].replay_buffer.store(step, all_observation[agent], all_observation_value[agent], all_action[agent], all_action_logprob[agent], all_reward[agent], all_done[agent], all_truncated[agent])
        ...
        for agent in range(para.num_agent):
            observation_value = agent_network[agent].get_observation_value(all_observation[agent])
            agent_network[agent].replay_buffer.store_last_state_value(observation_value)

        total_episodes += 1

        if total_episodes > para.mini_batch_size and episode % para.update_frequency == 0:
            for agent in range(para.num_agent):
                agent_network[agent].update(update_steps)
                update_steps += 1
